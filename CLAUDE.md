...
スタンプ風ラベルを付けている。配色・フォントは`style.css`冒頭の`:root`変数
（`--paper` `--ink` `--accent` 等）で管理。フォントはFraunces（見出し）/
IBM Plex Mono（メタ情報）/ Inter（UI）をGoogle Fontsから読み込み。
このトーンを崩さないよう、新規UIを追加する際は既存の変数を使うこと。
